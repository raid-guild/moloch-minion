...
/*   beforeEach(async () => { */
/*     molochMock = (await deployContract(wallet, MolochMockArtifact)) as MolochMock; */
/*     minion = (await deployContract(wallet, MinionArtifact, [molochMock.address])) as Minion; */
...
/*     let tester: Tester; */
/*     tester = (await deployContract(wallet, TesterArtifact, [molochMock.address, minion.address, target.address])) as Tester; */
/*     await tester.doTest(); */
/*   }); */
...
/*     const addProposalFuncSig = molochMock.interface.functions.addProposal.sighash; */
/*     const addProposalFuncData = ethers.utils.defaultAbiCoder.encode( */
/*       [ 'address', 'bytes' ], */
/*       [ minion.address, encodedMinionCall ] */
/*     ).slice(2); */
